...
struct MarkerStorage
{
	...
	//used in reconstruction, store the image coordinates of the markers
    std::vector<std::vector<Point2f> >imgPts;
	//used to select image
    std::vector<int> imgIndex;
	//used to store the world coordinates of 4 corners of the marker
    std::vector<Point3f> WPts;
	//used to store the marker's age for reconstruction
    std::vector<float> oldestTime;
    //the other format of world coordinates of 4 corners
	CvMat* WPt2;
	//camera parameters
    CvMat* camera_mat;
    ...
    int numImgs;
	//current smallest reconstruction error, used to find out better reconstruction
