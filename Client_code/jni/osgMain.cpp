...
void osgMain::draw()
{
	...
	//update rendering for the other user
...
void osgMain::initOSG(int width, int height)
{
	...
	//structure for geometry transformation
	...
	//use two triangles with yellow and red to represent the other user
	osg::Vec4 yellow;
	...
	viewer->frame();
	MFPCM->setEye(osg::Vec3(60.0,-250.0,20.0));
...
void osgMain::deleteCurrentSelectedGeometry()
{
	...
	{
		deleteGeode=false;
		__android_log_print(ANDROID_LOG_ERROR,"jni client","no such geode");
		return;
	}
