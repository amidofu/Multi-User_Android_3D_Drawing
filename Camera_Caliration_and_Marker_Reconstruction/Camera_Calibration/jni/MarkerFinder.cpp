...
int FindRectangle(IplImage* colorImg,MarkerRectangle* rectangles,const int rectLimitCount)
{
	...
	//__android_log_print(ANDROID_LOG_DEBUG,"Find Rectangle","MarkerRectangle initialized");
	...
	*/
	//__android_log_print(ANDROID_LOG_DEBUG,"Find Rectangle","Poly Contour OK");
#ifdef VERBOSE
	...
	//__android_log_print(ANDROID_LOG_DEBUG,"Find Rectangle","Compute Process Time");
...
void DecodeMarker2DCode(IplImage* colorImg,MarkerRectangle* rectangles,const int rectCount)
{
	...
	//__android_log_print(ANDROID_LOG_INFO,"Decode Marker","start");
	...
	//__android_log_print(ANDROID_LOG_INFO,"Decode Marker","OK1");
	...
	for(rectIndex=0;rectIndex<rectCount;rectIndex++)
	{
		...
		//__android_log_print(ANDROID_LOG_INFO,"Decode Marker","OK2, direction %d",Direction);
	...
	//__android_log_print(ANDROID_LOG_INFO,"Decode Marker","finish");
...
int CheckDirection(IplImage* projectedImg)
{
	...
	case DIRECTION_TL:
		...
		ROI.x=StartX;
		ROI.y=StartY;
		p1=cvPoint(ROI.x,ROI.y);
		p2=cvPoint(ROI.x+ROI.width,ROI.y+ROI.height);
		cvRectangle(projectedImg,p1,p2,Color,thickness,CV_AA,shift);
		break;
	case DIRECTION_BL:
		ROI.x=StartX;
		ROI.y=ceil(projectedImg->height*(float)(INNER_END/MARKER_OUTER_SIZE))-ROISize;
	...
	const int rectCount)
{
	...
	for(MarkerIDX=0;MarkerIDX<rectCount;MarkerIDX++)
	{
		...
		*/
		...
		tx=CV_MAT_ELEM(*EstimateTranslation_vector,float,0,0);
		ty=CV_MAT_ELEM(*EstimateTranslation_vector,float,0,1);
		tz=CV_MAT_ELEM(*EstimateTranslation_vector,float,0,2);
		rectangle[MarkerIDX].translation_vector=cvPoint3D32f(tx,ty,tz);
		...
		rx=CV_MAT_ELEM(*EstimateRotation_vector,float,0,0);
		ry=CV_MAT_ELEM(*EstimateRotation_vector,float,0,1);
		rz=CV_MAT_ELEM(*EstimateRotation_vector,float,0,2);
		rectangle[MarkerIDX].rotation_vector=cvPoint3D32f(rx,ry,rz);
	...
	const int rectCount)
{
	...
	for(i=0;i<4;i++)
	{
		...
		//cvmSet(RealAxisPoints3D,0,i,0);
		//cvmSet(RealAxisPoints3D,1,i,0);
		//cvmSet(RealAxisPoints3D,2,i,0);
		//cvmSet(ImgAxisPoints2D,0,i,0);
		//cvmSet(ImgAxisPoints2D,1,i,0);

	}


	//cvmSet(RealAxisPoints3D,0,1,MARKER_OUTER_SIZE/2);//X軸
	//cvmSet(RealAxisPoints3D,1,2,MARKER_OUTER_SIZE/2);//Y軸
	//cvmSet(RealAxisPoints3D,2,3,MARKER_OUTER_SIZE/2);//Z軸
	CV_MAT_ELEM(*RealAxisPoints3D,float,1,0)=MARKER_OUTER_SIZE/2;
	CV_MAT_ELEM(*RealAxisPoints3D,float,2,1)=MARKER_OUTER_SIZE/2;
	CV_MAT_ELEM(*RealAxisPoints3D,float,3,2)=MARKER_OUTER_SIZE/2;
	...
	for(rectIndex=0;rectIndex<rectCount;rectIndex++)
	{
		...
		cvProjectPoints2(corners3D,
			...
			NULL,NULL,NULL,NULL,NULL,0);
		CvMat* trans=cvCreateMat (1, 2, CV_32FC1);
		cvProjectPoints2(translation_vector,
			rotation_vector,translation_vector,
			camera_matrix,distortion_coeffs,trans,
			NULL,NULL,NULL,NULL,NULL,0);
		//てすと
		...
		*/
        ...
        cir.x=(int)CV_MAT_ELEM(*trans,float,0,0);
        cir.y=(int)CV_MAT_ELEM(*trans,float,0,1);
        cvCircle(colorImg,cir,7,CV_RGB(0,255,255),2,8,0);
        cvReleaseMat(&trans);
	}
	...
	MarkerRectangle & rectangle)
{
	...
	CvPoint cir;
		...
		//cvmSet(rotation_vector,0,1,rectangle[rectIndex].rotation_vector.y);
		//cvmSet(rotation_vector,0,2,rectangle[rectIndex].rotation_vector.z);
		CV_MAT_ELEM(*rotation_vector,float,0,0)=rectangle.rotation_vector.x;
		CV_MAT_ELEM(*rotation_vector,float,0,1)=rectangle.rotation_vector.y;
		CV_MAT_ELEM(*rotation_vector,float,0,2)=rectangle.rotation_vector.z;


		//cvmSet(translation_vector,0,0,rectangle[rectIndex].translation_vector.x);
		//cvmSet(translation_vector,0,1,rectangle[rectIndex].translation_vector.y);
		//cvmSet(translation_vector,0,2,rectangle[rectIndex].translation_vector.z);
		CV_MAT_ELEM(*translation_vector,float,0,0)=rectangle.translation_vector.x;
		CV_MAT_ELEM(*translation_vector,float,0,1)=rectangle.translation_vector.y;
		CV_MAT_ELEM(*translation_vector,float,0,2)=rectangle.translation_vector.z;
		...
		*/
        ...
        cir.x=(int)CV_MAT_ELEM(*corners2D,float,0,0);
        cir.y=(int)CV_MAT_ELEM(*corners2D,float,0,1);
        cvCircle(colorImg,cir,7,CV_RGB(255,0,0),5,8,0);
